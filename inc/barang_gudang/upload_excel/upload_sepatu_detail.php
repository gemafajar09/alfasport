...
if (isset($_POST['csv_sepatu_detail'])) {
    $file = fopen($_FILES['upload_sepatu_detail']['tmp_name'], 'r');
    $no = 0;
    while (!feof($file)) {
        ...
        if ($no > 0) {
            if (!empty($isi_baris[2])) {
                ...
                // cari ukuran_id
                $ukuran = $con->query("SELECT ukuran_id FROM tb_ukuran WHERE merk_id = '$merk_id' AND subdivisi_id = '$subdivisi_id' AND sepatu_ue = '$isi_baris[4]' AND ukuran_kategori = 'Sepatu'")->fetch();
                ...
                if ($ukuran_id != null and $barang_id != null) {
                    ...
                    $brc = $con->query("SELECT barang_detail_id, barang_detail_jml 
                                        ...
                                        AND 
                                        tb_barang.barang_kategori = 'Sepatu'
                                        AND 
                    ...
                    if (!empty($barang_detail_id)) {
                        $con->update(
                            ...
                            array(
                                ...
                                'barang_detail_jml' => $brc['barang_detail_jml'] + $isi_baris[8],
                                'barang_detail_tgl' => $isi_baris[9]
                            ),
                            array(
