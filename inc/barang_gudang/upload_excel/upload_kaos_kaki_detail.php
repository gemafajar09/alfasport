...
if (isset($_POST['csv_kaos_kaki_detail'])) {
    $file = fopen($_FILES['upload_kaos_kaki_detail']['tmp_name'], 'r');
    $no = 0;
    while (!feof($file)) {
        ...
        if ($no > 0) {
            if (!empty($isi_baris[2])) {
                ...
                // cari ukuran_id
                $ukuran = $con->query("SELECT ukuran_id FROM tb_ukuran WHERE merk_id = '$merk_id' AND subdivisi_id = '$subdivisi_id' AND kaos_kaki_eu = '$isi_baris[4]' AND ukuran_kategori = 'Kaos Kaki'")->fetch();
                ...
                if ($ukuran_id != null and $barang_id != null) {
                    ...
                    $brc = $con->query("SELECT barang_detail_id, barang_detail_jml 
                                        ...
                                        AND 
                                        tb_barang.barang_kategori = 'Kaos Kaki'
                                        AND 
