    ...
    <div class="x_content table-responsive">
        <div class="col-sm-6 col-md-6 col-lg-6">
            <div class="form-group">
                <label>Artikel Barang Lainnya</label>
                <select name="artikel" id="artikel" class="form-control select2">
                    <option value="">-Artikel Barang Lainnya-</option>
                    <?php
                    $artikel = $con->select('tb_gudang_lainnya', '*');
                    foreach ($artikel as $a) {
                    ?>
                        <option value="<?= $a['gudang_lainnya_id'] ?>"><?= $a['gudang_lainnya_nama'] ?> - <?= $a['gudang_lainnya_artikel'] ?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
...
<script>
    $('#toko').change(function(e) {
        ...
        axios.post('inc/cari_lainnya/filter/toko.php', {
            'toko': toko,
            'artikel': artikel
        }).then(function(res) {
            ...
            if (data.detail != false) {
                ...
                $('#namaartikel').text(data.detail.gudang_lainnya_artikel);
                $('#namabarang').text(data.detail.gudang_lainnya_nama);
                $('#modal').text(data.detail.gudang_lainnya_modal);
                $('#jual').text(data.detail.gudang_lainnya_jual);
